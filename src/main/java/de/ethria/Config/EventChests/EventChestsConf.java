...
public class EventChestsConf implements Listener {

    private String folderPath = "plugins/EthriaAddons/Addons/EventChests";
    private String configName = "eventchests.yml";
    private String folderPathBackup = "plugins/EthriaAddons/backups";
    private boolean Backup;

    public EventChestsConf() {
        createFolder();
        createYML();
    }
    ...
    public void createYML() {
        ...
        if(!file.exists()) {
            try (FileWriter fw = new FileWriter(file)) {
                fw.write("# Einstellungen für die EventChests #\n");
                fw.write("# (true = an | false = aus) #\n");
                fw.write("Enabled: 'true'\n");
                fw.write("Perm: 'system.ethria.eventchestsadmin'\n");
            } catch (IOException e) {
        ...
        if(wertContent == null) {
            switch (wert) {
                case "Enabled":
                    ...
                    break;
                case "Perm":
                    wertContent = "system.ethria.eventchestsadmin";
                    break;
            }
        }
        return wertContent;
